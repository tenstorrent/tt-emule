...
# Ported from run_ttnn_pytests_wormhole.sh.  Some of these will be skipped
# (or removed) for Blackhole based on tt-metal-side skip markers; failures
# are tracked in `known-failures-blackhole-pytests.txt` rather than fixed
# in this branch — this is the preemptive BH bring-up sweep.
