...
case "$BOARD" in
  ...
  *) echo "unknown board '$BOARD' (use n300 or p300)"; exit 2 ;;
esac
[ -f "$DESC" ] || { echo "descriptor not found: $DESC"; exit 2; }

export TT_METAL_DIR=$TT TT_METAL_HOME=$TT TT_METAL_RUNTIME_ROOT=$TT
export PYTHONPATH=$TT/ttnn:$TT/tools:$TT/build_emule/lib:$TT
export LD_LIBRARY_PATH=$TT/build_emule/lib
...
P=/opt/ttmlir-toolchain/venv/bin/pytest
