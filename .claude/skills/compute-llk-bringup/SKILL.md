...
## Index-based ops (TopK, Sort, Argmax) — verified playbook

These ops return **values + indices**. The math is done in f32 on emule and
in the SFPU on silicon; on value ties the index assignment can differ yet still
be correct. tt-metal's tests are designed around this — match the test's
contract, not silicon's exact index choice.

### What `test_topk.py` actually asserts
1. `assert_numeric_metrics` on **values** (PCC 0.9999, atol/rtol 1e-6).
2. `assert_equal` on **values** vs `torch.topk(sorted=True)` — **exact**. Because
   bf16 round-trips losslessly (CB bf16 → f32 DST → bf16), the correct top-K
   *values* match torch exactly regardless of which tied index was chosen. So
   the only requirement is: select the correct top-K **set**, sorted.
3. **Gather cosine** on indices: `cosine(torch_vals, gather(input, ttnn_idx)) > 0.99`.
   ...
   robust to tie-breaks. ⇒ indices only need to be *valid* (point at the right
   values), not identical.

So "correct but non-identical to silicon" is a non-issue here: get the top-K
values right (sorted) and valid indices, and the test passes. A throwaway
standalone probe (call `ttnn.topk`, then `torch.equal` on values +
`CosineSimilarity` on `gather(input, indices)`) localizes failures fast.

### Sort axis — the #1 gotcha
The ttnn topk compute kernels run `transpose_wh_tile` on every value/index tile
*before* the LLK calls, moving the sort dimension W onto the **row** index of
the 32×32 DST tile (`DST[r*32+c] = input[H=c][W=r]`). Top-k along W for fixed
H=c therefore compares datums across **rows** at a fixed **column** → each
column is an independent sort sequence. A per-row implementation returns garbage.

### Kernel paths and the three LLK primitives
...
emule shims live in `compute_kernel_api.h` (`namespace __emule_topk`:
`merge_split_col` = full-sort 64 (V0∪V1) keep top-32→V0 / bottom-32→V1;
`sort_col` = sort one tile's 32 per column):
- `topk_local_sort(idir, i_end_phase)`: `i_end_phase>=5` → `merge_split_col`
  (one 64-run, top-32→V0). `i_end_phase<5` → independent per-tile `sort_col`
  (each tile a separate local candidate set).
- `topk_merge<top_min>` **and** `topk_rebuild`: BOTH are "move the larger 32
  into V0, lower 32 into V1" (driver comments in `topk_common_funcs.hpp:100,160`)
  → both implemented as `merge_split_col`. The full 64-sort is order-independent,
  so the kernel's pairwise reduction tree (process_tiles) + the process_tile_pair
  merge converge to the global top-K for **all** K (32, 64, 3000) without needing
  to reproduce the SFPU's exact bitonic intermediate state or its alternating
  directions. (An element-wise compare-exchange merge is *also* what silicon
  does, but it requires getting the bitonic direction bookkeeping exactly right;
  the full-sort form is simpler and equally correct for the *final result*.)

### Infra bugs this op surfaced (all general, not topk-specific)
1. **`__emule_pack_offset` must reset on `cb_push_back`, not `cb_reserve_back`**
   (`cb_api.h`). The multi-core final kernel fills a staging CB with
   `reserve_back(1)` per tile in a loop + a single trailing `push_back(Wt)`,
   relying on auto-advancing `pack_tile(0,cb)` to hit distinct slots. A
   per-reserve reset collapsed all Wt tiles onto slot 0. **Highest-impact fix.**
2. **uint16 unpack branch missing the `n > __EMULE_TILE_ELEMS` clamp**
   (`common.h::__emule_unpack_cb_tile_to`). A non-standard oversized index page
   (2080-byte tiles on the dim=1 path) indexed `rowmajor_to_nfaces[]` past its
   1024 entries → OOB segfault. The 32-bit branch already clamped; the uint16
   one didn't.
3. **Program-global CB addressing** (`emulated_program_runner.cpp::init_core_cb_sync`):
   configure every program CB on every core at its global L1 address, so a
   kernel can `get_write_ptr(cb)` a CB allocated only on a *remote* core (the
   multi-core topk writers NOC-write into the final core's `final_*_cb`).
4. **`dataflow_api.h` must include `hostdevcommon/common_values.hpp`**
   (INVALID/VALID), mirroring silicon's transitive include.
5. **Self-recycled CB reserve** (`cb_api.h`): `cb_reserve_back` is non-blocking
   for a CB the same (compute) thread also consumes (`cb_wait_front` marks it in
   `__emule_cb_self_consume_mask`) — the in-place bitonic recycle idiom
   (`wait_front; <in-place pack_tile<true>>; reserve_back; pop_front; push_back`)
   deadlocks otherwise because emule runs compute single-threaded (silicon
   overlaps UNPACK/PACK on separate threads).

### Debug method that worked
Instrument the dataflow/compute boundary with `fprintf` in the JIT kernels and
emule shims (they run as host threads — `fprintf(stderr,...)` works): dump
per-tile values at the final-core input copy and after the merge reduction to
localize "data present but not surfaced" (a merge/pack bug) vs "data missing"
(an inter-core transfer bug). Here the final core *received* all cores' data
(global max present) but the staging fill collapsed it → pinpointed the
pack-offset bug.
