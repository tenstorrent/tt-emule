...
- **Use `-x` while iterating on a single kernel's clang errors.** `pytest <test>::<variant> -v --tb=short -s -x` stops on first fail and surfaces clang stderr immediately. Critical when a kernel takes several rounds of shim additions — without `-x`, pytest grinds through 100+ variants that all fail on the same compile error, costing ~5 minutes per round when you only need the first failure's diagnostic. Drop `-x` for the final post-fix sweep that confirms broad-PASS.
- **Constexpr-format-array gotcha.** When a tt-metal kernel-lib helper does `constexpr auto format = pack_dst_format[icb]` or `unpack_src_format[icb]`, the corresponding arrays in `jit_hw/internal/llk_state.h` must be `inline constexpr` (not `inline uint32_t = {}`). Real device codegen emits them as constexpr with per-CB `DataFormat` enum values; in emule, default-zero constexpr arrays work because DST is always fp32 (== 0 in the DataFormat enum). Symptom: `error: constexpr variable 'format' must be initialized by a constant expression`. The arrays are stubs — no emule code writes to them, so the constexpr change is safe.
- **DON'T implement `tilize_block` as `copy_tile + pack_tile`.** It compiles cleanly and looks symmetric, but it's wrong. `copy_tile` calls `__emule_unpack_cb_tile_to`, which **always** applies the nfaces→rowmajor permutation (it assumes the source CB holds nfaces tile bytes). For tilize, the source CB holds **row-major** bytes from the reader — applying the unpack permutation produces a scrambled DST. `pack_tile` then applies rowmajor→nfaces on the scrambled DST, so the round-trip is *not* identity. Symptom: PCC fails with very large ATOL (saw 191) — values look like they're pulled from unrelated positions. Fix: write `tilize_block` as a direct rowmajor→nfaces byte copy using `__emule_nfaces::rowmajor_to_nfaces` LUT — don't go through DST. Same trap applies if anyone writes `untilize_block` reusing `pack_tile` for a CB whose contents are tile-format; check whether the destination format expectation matches what the kernel-lib's caller passes.
