...
| PCC ≈ 0 with sane magnitudes (NO NaN) on a sharded-DRAM weight load — every non-zero bank reads zeros | Each DRAM bank lives in its OWN emule core's mmap (`SWEmuleChip::get_core` → `CoreRole::DRAM`, 2 GB each). `__emule_dram_ptr` aliases every bank to one core; sub-bank offsets within a core (0x40000000 for odd banks on Wormhole) are also ignored. | Use `__emule_dram_ptr_at(bank_id, offset)` in tt-metal's `emulated_program_runner.cpp` — resolves via `dram_bank_to_noc_xy[]` + `bank_to_dram_offset[]`. Switch the `noc_traits_t<AllocatorBank<DRAM>>` specialisation in tt-emule's `endpoints.h` to call it. Requires a tt-metal commit + pin bump. |
| PCC plateaus at 0.3-0.8 with reasonable magnitudes on a Bfp8 path | Off-by-N exponent bug in the Bfp8 decoder. Real Bfp8 stores RAW 7-bit unsigned mantissas (no implicit leading 1). The decoder must normalize: shift left until bit 6 is set, decrement exp by shift count, then strip the leading 1. Skipping normalization makes every value's exponent too high by `shift_cnt`. | Decoder template: scan the 7-bit mantissa for bit 6; if not set, shift left while counting; subtract count from exp; `(mant_norm << 1) & 0x7F` strips the implicit 1. Encoder mirror: place the leading 1 at bit 6 of the 7-bit raw mantissa = right-shift the aligned 24-bit significand by **17** (not 16). Validate end-to-end with a standalone host binary that round-trips against tt-metal's `pack_as_bfp8_tiles` / `unpack_bfp8_tiles_into_float_vec` — 10-line test, bit-exact lane-by-lane comparison. |
