# Implementation Report v3: Software-Emulated Device (tt-emule) Integration into tt-metal
...
The pool is sized at 2× the Tensix core count from the SOC descriptor to provide headroom for cores created via virtual coordinates that differ from physical coordinates.
...
```
tt-emule/                           (~5,990 lines across 87 files)
├── include/
...
│   ├── jit_hw/         (55 files)  JIT kernel stubs (resolved via dlopen + -rdynamic)
│   │   ├── api/compute/            26 SFPU/compute op headers
...
The `jit_hw/` directory provides stub implementations for 55 header files covering:
...
| Compute | 26 | `matmul_tiles`, `matmul_block`, `add/sub/mul_tiles`, `pack_tile`, `copy_tile`, 15 SFPU ops (INT32 math, bitwise, comparisons, etc.) |
...
| Compatibility | 12 | `ckernel.h`, `ckernel_defs.h`, `common_values.hpp`, `risc_attribs.h`, etc. |
...
| 5 | TTNN Matmul Sweep | 1 | 14 sub-cases: multi-core matmul 32×32×32 through 2048×2048×2048 (Wormhole N150) |
...
└── chip/sw_emulated_chip.cpp                   (112 lines)
```
...
- **L1Pool for worker cores.** At construction, allocates a single contiguous `MAP_32BIT` mmap with 2× the Tensix core count slots (from `soc.get_cores(CoreType::TENSIX).size()`). Worker cores get their L1 from pool slots; DRAM cores use individual mmaps. Pool exhaustion falls back to individual `MAP_32BIT` mmaps.
...
└── emulated_program_runner.cpp  (674 lines)
```
...
Phase 2: Execute all cores concurrently
    For each physical core:
        ...
        3. Set thread-local bridge pointers:
        ...
        4. Launch one thread per kernel (reader + compute + writer)
        5. Join all threads
        6. Done — no copy-back, no munmap
```
...
35 call sites across 16 files guard hardware-specific operations:
...
| Context init | `metal_context.cpp` | 3 | Fabric, FW, watcher, dprint |
...
User Code (ttnn::matmul, ttnn::add, etc.)
    └── tt-metal API (CreateBuffer, CreateProgram, LaunchProgram)
            └── [TT_METAL_EMULATION guard] execute_program_emulated()
                    ...
                    └── Launch N cores × M threads, set TLS context
...
tt_emule::L1Pool
    ├── Single contiguous MAP_32BIT mmap (num_slots × 2MB + alignment overhead)
...
| 5 | test_ttnn_matmul_sweep | 14 | Multi-core matmul with semaphore sync (32²–2048²) | Wormhole N150 |
...
Regression script: `tt_emule/run_regression.sh` — runs all 18 tests, reports pass/fail/skip counts, exits non-zero on any failure.
...
|----------|-------|---------------------|
| New: program runner | `emulated_program_runner.{hpp,cpp}` | ~723 |
| New: UMD chip | `sw_emulated_chip.{hpp,cpp}` | ~182 (with L1Pool integration) |
| New: test infrastructure | `tt_emule/CMakeLists.txt`, `tt_emule/ttnn_tests/CMakeLists.txt`, `tt_emule/run_regression.sh` | ~246 |
| Modified: dispatch | `tt_metal.cpp` | ~20 |
| Modified: guards | 16 files (`is_mock_or_emulated()`) | ~35 one-liners |
...
**Development velocity without hardware.** Developers can write, compile, and test kernels on any x86 Linux machine. The full loop — kernel compilation, buffer I/O, multi-threaded multi-core execution with CB synchronization and NOC communication — runs in seconds without requiring a Tenstorrent card. The matmul sweep (14 cases up to 2048×2048) completes in under 30 seconds.
...
**Incrementally extensible.** New compute ops are single-file headers in `jit_hw/api/compute/`. The pattern (DST-to-DST operations with format-aware load/store) has been applied consistently across 26 compute headers.
...
**JIT compilation overhead.** Each unique kernel requires a `g++` invocation (typically 1–3 seconds). Results are cached within a process but not persisted across runs. Test suites with many distinct kernels pay the full compilation cost on every invocation.

**`MAP_32BIT` address space pressure.** The JIT path relies on `mmap` with `MAP_32BIT` to ensure L1 pointers fit in `uint32_t`. The 32-bit address space region (~2 GB) is shared across all cores. L1Pool mitigates this by consolidating all worker L1 into a single contiguous allocation (Wormhole 80 cores × 2 MB = 160 MB, Blackhole 140 cores × 2 MB = 280 MB — both fit easily). DRAM cores use regular mmap outside the 32-bit region. The remaining risk is that non-pool fallback cores (from pool exhaustion) still compete for the same 32-bit space.
...
| Aspect | v2 | v3 |
|--------|----|----|
| Codebase size | ~4,660 lines, 65 files | ~5,990 lines, 87 files |
| JIT stubs | 33 files | 55 files |
| Program runner | 393 lines | 674 lines (multi-core NOC, banking, multicast) |
| NOC communication | Single-core only | Full cross-core: multicast writes, semaphore inc, NOC address resolution across core map |
| Semaphore support | Declared but untested | Fully functional: get_semaphore returns host pointer, noc_semaphore_set/wait/wait_min/inc, multicast semaphore ops |
| Address translation | Implicit (only CB pointers) | Three-point model: `__emule_addr_to_offset` (encode, bitmask via L1Pool), `__emule_fixup_noc_addr` (fixup OR-constructed addrs), `__emule_resolve_noc_addr` (decode) |
| Memory allocation | Per-core individual `MAP_32BIT` mmap | L1Pool: single contiguous mmap with 2MB-aligned slots; DRAM cores still individual mmap |
| Banking | Single-bank stub | Full interleaved DRAM/L1 banking from SOC descriptor |
| Matmul | Single-core `matmul_tiles` only | Multi-core: `matmul_block`, `pack_tile_block`, sender/receiver with semaphore sync |
| Compute stubs | 15 SFPU headers | 26 compute headers (+ `matmul_block`, `pack_untilize`, `transpose_wh`, `bcast`, `quantization`) |
| Test coverage | 25 tests (eltwise only) | 18 regression entries covering ~100 sub-cases (eltwise, relational, matmul sweep) |
| Cluster support | Blackhole only | Wormhole N150 + Blackhole P100 (per-tier cluster selection) |
| Regression | Manual | `run_regression.sh` with 5 tiers, pass/fail/skip reporting |

---

*Report updated 2026-03-11. Covers tt-emule at commit `4842149` + L1Pool refactor (master) / tt-metal commit `fbd48e0da5` + L1Pool integration (xchin/tt-emule branch).*
