...
#
# emule env wrapper for perf/oneshot_lifecycle.py. Runs the one-shot lifecycle
# measurement at a series of --ops counts so total(N) can be compared against a
# silicon run to find the cold-start crossover.
#
#   perf/run_oneshot_emule.sh [OUT_CSV] [OPS_LIST...]
#   perf/run_oneshot_emule.sh /tmp/emule_oneshot.csv 1 2 5 10 20 50
#
# Each invocation is a FRESH process (a real one-shot: pays import + open every
# time). Required env: TT_METAL_DIR. Optional: BUILD_DIR, PYTHON_BIN, ARCH, OP.
...
OUT="${1:-/tmp/emule_oneshot.csv}"; shift || true
OPS_LIST=("$@"); [ ${#OPS_LIST[@]} -eq 0 ] && OPS_LIST=(1 2 5 10 20 50)
...
rm -f "$OUT"
for n in "${OPS_LIST[@]}"; do
    echo "== emule one-shot: ops=$n =="
    "$PYTHON_BIN" "$SCRIPT_DIR/oneshot_lifecycle.py" \
        --backend "emule-$ARCH" --op "$OP" --ops "$n" --out "$OUT"
done
echo "== wrote $OUT =="
