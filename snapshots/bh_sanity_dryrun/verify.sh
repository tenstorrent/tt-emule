#!/usr/bin/env bash
# DRY-RUN Phase 8 verify checks. Exits 0 if all checks pass, 1 otherwise.

set -uo pipefail

OUT_DIR="/localdev/arminale/tt-emule/snapshots/bh_sanity_dryrun"
EXEC="$OUT_DIR/dryrun_status_exec.md"
DEV="$OUT_DIR/dryrun_status_dev.md"
XML="$OUT_DIR/test_pad.xml"
LOG="$OUT_DIR/test_pad.log"
AUDIT="/localdev/arminale/tt-emule/snapshots/bh_sanity/audit_bh_sanity.log"

JARGON_TOKENS=(wave)

fail=0
ok()   { printf '  \033[32mPASS\033[0m  %s\n' "$*"; }
bad()  { printf '  \033[31mFAIL\033[0m  %s\n' "$*"; fail=$((fail+1)); }

section() { echo; echo "== $* =="; }

section "Artefacts present"
for f in "$EXEC" "$DEV" "$XML" "$LOG"; do
    if [ -s "$f" ]; then ok "$f"; else bad "$f missing or empty"; fi
done

section "XML well-formed"
python3 -c "import xml.etree.ElementTree as ET; ET.parse('$XML')" 2>/dev/null \
    && ok "$XML parses" || bad "$XML fails to parse"

section "Banner present"
grep -q "DRY RUN" "$EXEC" && ok "exec banner" || bad "exec banner missing"
grep -q "DRY RUN" "$DEV" && ok "dev banner" || bad "dev banner missing"

section "Jargon scan on exec"
for tok in "${JARGON_TOKENS[@]}"; do
    c=$(grep -ic "\\b${tok}\\b" "$EXEC" 2>/dev/null)
    c=${c:-0}
    if [ "$c" = "0" ]; then ok "no '$tok' (count=$c)"; else bad "found '$tok' x$c in exec"; fi
done

section "Section coverage"
exec_sections=("## Headline" "## Status" "## What \"skipped by design\"" "## Top 3 gaps" "## Top 3 next steps" "## Variant readiness" "## Recent landmarks")
for s in "${exec_sections[@]}"; do
    if grep -qF "$s" "$EXEC"; then ok "exec has '$s'"; else bad "exec missing '$s'"; fi
done
dev_sections=("## Headline" "## Fully-passing files" "## Partial-pass files" "## All-failing files" "## Blocker taxonomy" "## Coverage analysis" "## Variant readiness" "## Top suggested next steps" "## References")
for s in "${dev_sections[@]}"; do
    if grep -qF "$s" "$DEV"; then ok "dev has '$s'"; else bad "dev missing '$s'"; fi
done

section "Audit log integration"
if [ -s "$AUDIT" ]; then
    ok "audit log exists"
    tail -1 "$AUDIT" | grep -q "AUDIT PASS" && ok "audit ends with AUDIT PASS" || bad "audit not AUDIT PASS"
    grep -qF "$AUDIT" "$DEV" && ok "dev references audit log path" || bad "dev does not reference audit log path"
else
    bad "audit log missing"
fi

section "Path resolution (dev report)"
# Pull out backticked filesystem paths. Exclude pytest node IDs (`::` separator)
# and parametrize IDs (`[…]`) — those aren't file paths.
missing=0
while IFS= read -r p; do
    case "$p" in
        *"::"*|*"["*) continue ;;
    esac
    if [ -e "$p" ] || [ -e "/localdev/arminale/tt-emule/$p" ] || [ -e "/localdev/arminale/tt-metal/$p" ]; then
        continue
    fi
    bad "unresolved path in dev: $p"
    missing=$((missing+1))
done < <(grep -oE '`[^`]+`' "$DEV" | tr -d '`' | grep -E '^(/|snapshots/|tests/|docs/|scripts/)' | sort -u)
[ "$missing" = "0" ] && ok "all cited paths resolve" || true

section "Totals consistency (headline P vs XML)"
# Aggregate across every XML in the output dir — multi-file dry-runs and the
# eventual full-sweep both land here.
xml_p=$(python3 -c "
import xml.etree.ElementTree as ET, glob
p = 0
for path in sorted(glob.glob('$OUT_DIR/*.xml')):
    root = ET.parse(path).getroot()
    suites = root.findall('testsuite') if root.tag != 'testsuite' else [root]
    for s in suites:
        for tc in s.findall('testcase'):
            if tc.find('failure') is None and tc.find('error') is None and tc.find('skipped') is None:
                p += 1
print(p)")
hdr_p=$(grep -oE '\| Tests passing \| [0-9]+ \|' "$EXEC" | grep -oE '[0-9]+' | head -1)
if [ "$xml_p" = "$hdr_p" ]; then
    ok "headline P=$hdr_p matches aggregated XML P=$xml_p"
else
    bad "headline P=$hdr_p but aggregated XML P=$xml_p"
fi

echo
if [ "$fail" = "0" ]; then
    echo "VERIFY PASS"
    exit 0
else
    echo "VERIFY FAIL ($fail check(s) failed)"
    exit 1
fi
