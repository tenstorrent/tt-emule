...
// TRISC sync primitives — referenced by the verbatim-inlined body of
// `experimental::unpack_stall_on_pack` (D2M emits the contents of
// `experimental_reg_api.h` into the kernel). On real silicon these gate
// PACK/UNPACK threads on TRISC semaphores; on emule (single-threaded
// compute) they are no-ops.
namespace semaphore {
...
ALWI void t6_semaphore_post(int /*sem*/ = 0) {}
template <int Stall = 0>
ALWI void t6_semaphore_wait_on_zero(int /*sem*/ = 0) {}
template <int Stall = 0>
ALWI void t6_semaphore_get(int /*sem*/ = 0) {}
