...
//
// The `semaphore` and `p_stall` namespaces are owned by `ckernel.h` and
// `api/compute/matmul_fused_act_emule.h` (canonical `ckernel::` definitions).
// They must NOT be redefined in the global namespace: `using namespace
// ckernel;` at the end of common.h brings the canonical versions to global
// scope, and a duplicate global definition causes ambiguous-lookup errors
// in any kernel that references `semaphore::*` or `p_stall::*`.
