...
// L1-side format arrays (unpack_src_format / pack_dst_format) are populated from the
// EMULE_CB_DATA_FORMATS JIT define in jit_hw/api/cb_api.h (single source of truth,
// mirroring the device's chlkc_descriptors.h). Pulled in here so kernel-lib helpers like
// `constexpr auto format = unpack_src_format[icb]` (tilize_helpers.inl is_fp32_input_format)
// resolve them as constant expressions.
#include "jit_hw/api/cb_api.h"
// DST-side format arrays stay stubbed: emule's DST register file is always fp32, so the
// unpacker-dst / packer-src formats are immaterial to the emulated pack/unpack math.
