...
// thread-global pack-fused ReLU clamp (`__emule_compute_ctx().pack_relu_mode` /
...
inline void __llk_pack_tiled(uint32_t tile_idx, uint32_t ocb) {
    ...
    uint8_t* buf = __emule_compute::cb_write_ptr_at(ocb, __emule_compute_ctx().llk_pack_offset);
    if (__emule_compute::cb_is_bfp4_b_format(ocb)) {
        ...
        for (uint32_t fr = 0; fr < 64; ++fr) {
            ...
            for (uint32_t k = 0; k < 16; ++k) {
                ...
                row16[k] = __emule_apply_pack_relu(__emule_compute_ctx().dst[tile_idx][rm]);
            }
            uint8_t packed[8];
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            uint32_t bits;
            std::memcpy(&bits, &__emule_compute_ctx().dst[tile_idx][i], sizeof(uint32_t));
        ...
        if (__emule_compute_ctx().pack_relu_mode == ReluType::NO_RELU) {
            ...
            for (uint32_t i = 0; i < n; i++) {
                uint32_t ni = __emule_nfaces::rowmajor_to_nfaces[i];
                std::memcpy(&out[ni], &__emule_compute_ctx().dst[tile_idx][i], sizeof(uint32_t));
            }
        } else {
            ...
            for (uint32_t i = 0; i < n; i++) {
                uint32_t ni = __emule_nfaces::rowmajor_to_nfaces[i];
                out[ni] = __emule_apply_pack_relu(__emule_compute_ctx().dst[tile_idx][i]);
            }
        }
    } else {
        ...
        for (uint32_t i = 0; i < n; i++) {
            uint32_t ni = __emule_nfaces::rowmajor_to_nfaces[i];
            bf[ni] = __emule_bf16::from_f32(__emule_apply_pack_relu(__emule_compute_ctx().dst[tile_idx][i]));
        }
    }
}
...
inline void __llk_pack_untilize(uint32_t tile_idx, uint32_t ocb) {
    ...
    const uint32_t block_c = __emule_compute_ctx().llk_pack_block_c;
    const uint32_t tile_row = __emule_compute_ctx().llk_pack_offset / block_c;
    const uint32_t tile_col = __emule_compute_ctx().llk_pack_offset % block_c;
    ...
    if (__emule_compute::cb_is_uint16_format(ocb)) {
        ...
        for (uint32_t r = 0; r < TILE_DIM; r++) {
            ...
            for (uint32_t c = 0; c < TILE_DIM; c++) {
                uint32_t bits;
                std::memcpy(&bits, &__emule_compute_ctx().dst[tile_idx][r * TILE_DIM + c], sizeof(uint32_t));
        ...
        if (__emule_compute_ctx().pack_relu_mode == ReluType::NO_RELU) {
            for (uint32_t r = 0; r < TILE_DIM; r++) {
                uint32_t* dst = reinterpret_cast<uint32_t*>(
                    base + tile_row_offset + r * row_stride + tile_col_offset);
                for (uint32_t c = 0; c < TILE_DIM; c++)
                    std::memcpy(&dst[c], &__emule_compute_ctx().dst[tile_idx][r * TILE_DIM + c], sizeof(uint32_t));
            }
        } else {
            for (uint32_t r = 0; r < TILE_DIM; r++) {
                float* dst = reinterpret_cast<float*>(
                    base + tile_row_offset + r * row_stride + tile_col_offset);
                for (uint32_t c = 0; c < TILE_DIM; c++)
                    dst[c] = __emule_apply_pack_relu(__emule_compute_ctx().dst[tile_idx][r * TILE_DIM + c]);
            }
        }
        return;
    }
    ...
    for (uint32_t r = 0; r < TILE_DIM; r++) {
        uint16_t* dst = reinterpret_cast<uint16_t*>(
            base + tile_row_offset + r * row_stride + tile_col_offset);
        for (uint32_t c = 0; c < TILE_DIM; c++)
            dst[c] = __emule_bf16::from_f32(__emule_apply_pack_relu(__emule_compute_ctx().dst[tile_idx][r * TILE_DIM + c]));
    }
}
...
inline void llk_pack(uint32_t tile_idx, uint32_t ocb) {
    if (__emule_compute_ctx().llk_pack_is_untilize) __llk_pack_untilize(tile_idx, ocb);
...
// explicit-offset path — honours the per-CB pack_width and advances
// __emule_compute_ctx().pack_offset[cb] so a following cb_push_back stays consistent.
template <int AccumMode, bool Untilize, ckernel::PackMode Mode>
inline void llk_pack(uint32_t dst, uint32_t cb, uint32_t idx) {
    const uint32_t w = __emule_compute_ctx().pack_width[cb] ? __emule_compute_ctx().pack_width[cb] : 1;
    ...
    if (idx + w > __emule_compute_ctx().pack_offset[cb]) __emule_compute_ctx().pack_offset[cb] = idx + w;
}
...
// pack_tile reproduces the multi-tile emit. See ComputeThreadCtx::pack_width.
...
// how many top rows of the DST tile are valid. (pack_rows_num lives in
// ComputeThreadCtx, reached via __emule_compute_ctx().pack_rows_num.)
inline void llk_pack_rows_init(uint32_t num_rows) {
    ...
    ASSERT(num_rows <= 32);
    __emule_compute_ctx().pack_rows_num = num_rows;
}
inline void llk_pack_rows_uninit() { __emule_compute_ctx().pack_rows_num = 32; }
inline void _llk_pack_rows_(uint32_t idst, uint32_t l1_addr) {
    ...
    const uint32_t n = __emule_compute_ctx().pack_rows_num * 32u;
    for (uint32_t i = 0; i < n && i < __EMULE_TILE_ELEMS; ++i) {
        bf[i] = __emule_bf16::from_f32(__emule_apply_pack_relu(__emule_compute_ctx().dst[idst][i]));
    }
}
