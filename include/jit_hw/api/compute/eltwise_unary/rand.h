...
ALWI void rand_tile_init(uint32_t seed = 0) {
    if (seed != 0) {
        ...
        __emule_compute_ctx().rand_engine.seed(seq);
        __emule_compute_ctx().rand_deterministic = true;
    } else {
        __emule_compute_ctx().rand_deterministic = false;
    }
}
...
ALWI void rand_tile(uint32_t idst, uint32_t from = 0, uint32_t scale = 0) {
    ...
    if (!__emule_compute_ctx().rand_deterministic) {
        std::random_device rd;
        std::seed_seq seq{rd(), rd(),
                          static_cast<uint32_t>(__emule_compute_ctx().rand_nonce & 0xFFFFFFFFu),
                          static_cast<uint32_t>(__emule_compute_ctx().rand_nonce >> 32),
                          get_absolute_logical_x(), get_absolute_logical_y()};
        __emule_compute_ctx().rand_engine.seed(seq);
        ++__emule_compute_ctx().rand_nonce;
    }

    std::uniform_real_distribution<float> dist(0.0f, 1.0f);
    for (uint32_t i = 0; i < __EMULE_TILE_ELEMS; i++) {
        __emule_compute_ctx().dst[idst][i] = from_f + dist(__emule_compute_ctx().rand_engine) * scale_f;
    }
}
