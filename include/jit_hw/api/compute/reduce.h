...
inline void reduce_tile(uint32_t icb, uint32_t icb_scaler,
    ...
    // UNPACK source tile: row-major float32 regardless of CB dtype
    // (Bfp8_b / 32-bit raw / bf16).
    float src[1024];
    __emule_unpack_tile_to_f32(icb, itile, src);

    // UNPACK scaler tile: scaler is uniform, but apply for correctness.
    float scaler_tile[1024];
    __emule_unpack_tile_to_f32(icb_scaler, itile_scaler, scaler_tile);
    float scaler = scaler_tile[0];
