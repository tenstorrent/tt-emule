...
ALWI void add_binary_tile(uint32_t idst0, uint32_t idst1, uint32_t odst) {
    for (uint32_t i = 0; i < __EMULE_TILE_ELEMS; i++)
        __emule_compute_ctx().dst[odst][i] = __emule_compute_ctx().dst[idst0][i] + __emule_compute_ctx().dst[idst1][i];
}
...
ALWI void rsub_binary_tile(uint32_t idst0, uint32_t idst1, uint32_t odst) {
    for (uint32_t i = 0; i < __EMULE_TILE_ELEMS; i++)
        __emule_compute_ctx().dst[odst][i] = __emule_compute_ctx().dst[idst1][i] - __emule_compute_ctx().dst[idst0][i];
}
...
ALWI void power_binary_tile(uint32_t idst0, uint32_t idst1, uint32_t odst) {
    for (uint32_t i = 0; i < __EMULE_TILE_ELEMS; i++)
        __emule_compute_ctx().dst[odst][i] = std::pow(__emule_compute_ctx().dst[idst0][i], __emule_compute_ctx().dst[idst1][i]);
}
...
ALWI void eq_binary_tile(uint32_t idst0, uint32_t idst1, uint32_t odst) {
    for (uint32_t i = 0; i < __EMULE_TILE_ELEMS; i++)
        __emule_compute_ctx().dst[odst][i] = (__emule_compute_ctx().dst[idst0][i] == __emule_compute_ctx().dst[idst1][i]) ? 1.f : 0.f;
}
