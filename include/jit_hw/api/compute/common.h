...
// Underlying type pinned to uint8_t to match the opaque declaration in
// emule_thread_ctx.h (lets ComputeThreadCtx hold a ReluType pack_relu_mode).
...
// per-compute-thread state in ComputeThreadCtx (reached via
// __emule_compute_ctx().pack_relu_mode / .pack_relu_threshold).
// `llk_pack_relu_config(ReluType)` writes the mode; `pack_set_relu_threshold(float)`
// the threshold. `pack_dst_to_buf` / `__llk_pack_tiled` / `__llk_pack_untilize`
// apply the clamp before format conversion.
...
inline void pack_dst_to_buf(uint8_t* buf, uint32_t dst_slot, uint32_t ocb) {
    ...
    if (__emule_pack_subrect_active()) {
        ...
        // an out-of-range config would index past __emule_compute_ctx().dst / `buf` and corrupt
...
ALWI void pack_tile(uint32_t idst, uint32_t ocb) {
    ...
    __emule_compute::pack_dst_to_buf(
        __emule_compute::cb_write_ptr_at(ocb, __emule_compute_ctx().pack_offset[ocb]++), idst, ocb);
}
...
ALWI void pack_tile(uint32_t idst, uint32_t ocb, uint32_t output_offset = 0) {
    ...
    if constexpr (UseOutputOffset) {
        ...
        if (output_offset >= __emule_compute_ctx().pack_offset[ocb]) {
            __emule_compute_ctx().pack_offset[ocb] = output_offset + 1;
        }
    } else {
        pack_tile(idst, ocb);
    }
...
// DST slot (regular copy_tile path) or __emule_compute_ctx().src_scratch (binary_dest_reuse_tiles
...
//
// Emule mirrors this with __emule_compute_ctx().src_scratch standing in for the
