...
// (Quasar uses `enum class`; BH/WH use unscoped `enum`. We use unscoped `enum`
// here to match BH/WH exactly — the scoped variant was the divergence.)
// Earlier emule stubs had RC=0 / R=1 / C=2 which would silently disagree with
// any code that compared `vector_mode` against the canonical integer constants.
//
// Caveat: unscoped `enum` leaks the enumerator names (None, R, C, RC,
// RC_custom, Invalid) into `namespace ckernel`, and through the repo's
// pervasive `using namespace ckernel` into global scope. `None`/`R`/`C` are
// short and collision-prone; TU authors need to be aware.
