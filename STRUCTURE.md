...
- `include/jit_hw/api/compute/ema.h` — `ckernel::` `ema_tile`, `ema_init`, `ema_clear_previous_output`; TLS `__emule_ema_{alpha,beta,prev,has_prev}`
...
- `include/jit_hw/api/dataflow/dataflow_api.h` — `noc_async_read`/`noc_async_write` (+ `_one_packet`, `_tile`), `noc_async_write_multicast`/`_multicast_loopback_src`, `noc_async_read_barrier`/`noc_async_write_barrier`/`noc_async_writes_flushed`/`noc_async_atomic_barrier`/`noc_async_full_barrier`, `noc_semaphore_set`/`_wait`/`_wait_min`/`_inc`/`_set_multicast`, `noc_inline_dw_write`, `dram_barrier`; `get_noc_addr`/`get_dram_noc_addr`/`get_l1_noc_addr`/`get_noc_multicast_addr`, `get_semaphore`; `__emule_addr_to_offset`/`__emule_fixup_noc_addr`/`__emule_resolve_noc_addr`/`__emule_local_l1_to_ptr`; `struct CBInterface`, `__emule_dw_state`; NOC VC/burst macros, `EMULE_SEM_BASE`
